...
class SynthesizerInfinite(Optimizer):
    ...
    def build_dynamics_descriptors(self, name, weight):
        with tf.get_default_graph().name_scope(name):
            ...
            return tf.multiply(self.style_loss_dy('dynamics_style_loss',
                                                  gramians), weight)
            ...
            for frame in range(len(gramians)):
                gramian_diffs = [
                    (tf.tile(avg_target_grams[layer],
                     ...
                     gramians[frame][layer][1:]) * (2 ** (5 - layer)) for layer in range(num_layers)]
    ...
    def style_loss_dy(self, name, gramians):
        with tf.get_default_graph().name_scope(name):
            num_layers = len(gramians[0])
            target_gramians = [[g[:1] for g in grams]
                               for grams in gramians[:-1]]  # exclude last one
